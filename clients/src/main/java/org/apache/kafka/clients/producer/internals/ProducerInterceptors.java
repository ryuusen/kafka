...
public class ProducerInterceptors<K, V> implements Closeable {
     ...
     */
    public void onAcknowledgement(ProducerRecord<K, V> producerRecord, RecordMetadata metadata, Exception exception) {
        for (ProducerInterceptor<K, V> interceptor : this.interceptors) {
            try {
                interceptor.onAcknowledgement(producerRecord, metadata, exception);
            } catch (Exception e) {
        ...
        for (ProducerInterceptor<K, V> interceptor : this.interceptors) {
            try {
                if (record == null && interceptTopicPartition == null) {
                    interceptor.onAcknowledgement(null, null, exception);
                } else {
                    ...
                    interceptor.onAcknowledgement(record, new RecordMetadata(interceptTopicPartition, -1, -1,
